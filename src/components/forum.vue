      ...
      <div class="forum-centercontent">
        <div class="forum-centercard">
          <el-card>
            ...
            <el-input class="forum-searchinput" v-model="search" placeholder="请输入关键词" size="small">
            </el-input>
            <el-button class="forum-searchbutton" type="primary" icon="el-icon-search" size="small"
              @click="forum_search_post">搜索
            </el-button>
          </el-card>
        </div>
        <div class="forum-centercard">
          <el-card>
            <div style="height:190px">
              ...
              <el-input type="text" v-model="forum_title" placeholder="请输入标题，不超过10个字" size="small" maxlength="10"
                show-word-limit clearable></el-input>
              <div style="margin: 20px 0;"></div>
              <el-input type="textarea" :rows="2" placeholder="请输入内容，不超过120个字" v-model="forum_text" maxlength="120"
                show-word-limit>
              </el-input>
              <div style="width:40%;float:left;margin-top:15px;margin-right:200px">
                <el-radio v-model="forum_radio" label="jointopic">参与话题</el-radio>
                <el-radio v-model="forum_radio" label="joincompetition">参与评选</el-radio>
              </div>
              <div class="message-buttom" style=" cursor: pointer;">
                ...
                <div class="upload_buttom">
                  <el-upload action="http://gulimall-nkhyx.oss-cn-beijing.aliyuncs.com" :data="dataObj"
        ...
        <div v-for="(item, index) in posts" :key="index" class="forum-centercard">
          <el-card>
            <div class="post">
              <div class="post-head">
                <div class="block">
                ...
                <div style="float:right;cursor: pointer;" @click="delete_post(item.id)" v-if="role == 2"><i
              ...
              <div class="post-content">
                ...
                <div class="forum-post-pic" style="text-align:left;margin-top:20px ;">
                  <img :src="item.pic1" alt="" style="width:150px;" v-show="item.pic1 != null && item.pic1 != ''" />

                  <img :src="item.pic2" alt="" style="width:150px;" v-show="item.pic2 != null && item.pic2 != ''" />
                </div>
              </div>

              <div class="post-judge" style="float:right">
                <span style="float: left; cursor: pointer;" @click="
                  show_comments(index),
                ...
                ">
                  <img src="../icons/svg/3.1 评论.svg" alt="" style="width:25px;margin-bottom: 0;" />
                  <span style="font-size:20px;float: right;margin-left: 5px;">{{
                      item.commentcount
                  }}</span>
                </span>
                <span style="margin-left: 10px; cursor: pointer;" @click="forum_post_addup(index)">
                  <img src="../icons/svg/爱心.svg" alt="" style="width:25px;margin-bottom: 0;"
                    v-if="!post_isliked[index].cut" />
                  <img src="../icons/svg/爱心-later.svg" alt="" style="width:25px;margin-bottom: 0;" v-else />
                  <span style="font-size:20px;float: right;margin-left: 5px;">{{
                      posts[index].up
                  }}</span>
                </span>
              </div>
              <div class="comments" style="height:auto" v-show="is_show_comment[index].cut">
                <div>
                  <el-input type="textarea" :rows="1" placeholder="发布你的评论" v-model="forum_comment" maxlength="60"
                    show-word-limit>
                  </el-input>
                </div>
                <div class="post-comments" style="text-align:right">
                  <el-button type="primary" size="medium" round @click="add_comments(posts[index].id, index)">
                    发送评论
                  </el-button>
                </div>

                <div class="show-comments">
                  <div class="comment-head" style="width:100%;height:50px"
                    v-for="(item, index1) in comments[index].data" :key="index1">
                    <div class="block">
                    ...
                    <div class="name-time" style="width:490px">
                      ...
                      <span>{{ item.text }}</span>
                      <div style="width:100%">
                        <span style="color:#939393;font-size: 10px;margin-top :10px;">{{ item.date }}</span>
                        <span class="post-judge" style="float:right;font-size: 15px;">
                          <span style="float: left;
                            cursor: pointer;" @click="dialogVisible = true">
                            <img src="../icons/svg/3.1 评论.svg" alt="" style="width:20px;margin-bottom: 0;"
                              @click="doopen(index, index1)" />
                            <span style="font-size:15px;float: right;margin-left: 5px;">{{ item.comments.length
                            }}</span>
                            <el-dialog title="" :visible.sync="dialogVisible" width="50%" :before-close="handleClose">
                              <div style="margin-bottom:20px;font-size:20px">
                                回复 @{{ cur_comment.username }}
                              </div>
                              <el-input type="text" placeholder="发布你的评论" v-model="forum_comments_for_comments"
                              ...
                              <span slot="footer" class="dialog-footer">
                                ...
                                <el-button type="primary" @click="
                                  (dialogVisible = false),
                                  add_comments_for_comments(posts[index].id, cur_comment.id)
                          ...
                          <span style="margin-left: 10px;
                            cursor: pointer;" @click="forum_comment_addup(index, index1)">
                            <img src="../icons/svg/爱心.svg" alt="" style="width:20px;margin-bottom: 0;"
                              v-if="!item.islike" />
                            <img src="../icons/svg/爱心-later.svg" alt="" style="width:20px;margin-bottom: 0;" v-else />
                            <span style="font-size:15px;float: right;margin-left: 5px;">{{ item.up }}</span>
                          </span>
                        </span>
                      </div>
                    </div>
                    <div class="comment-in-comment"
                      style="margin-left:40px;font-size: 13px;margin-bottom: 20px;height:auto;" v-for="(item, index2) in comments[index].data[index1]
                      .comments" :key="index2">
                      <div class="name-time" style="width:490px">
                        <span style="color:dodgerblue">
                          {{ item.username }}:</span>
                        <span>{{ item.text }}</span>
                        <div style="width:100%">
                          <span style="color:#939393;font-size: 8px;margin-top :10px;">{{ item.date }}</span>
                        </div>
                      </div>
                      <div></div>
                    </div>
        ...
        <div class="forum-post-page">
          <el-pagination @current-change="handleCurrentChange" :current-page="post_currentpage"
            :page-size="post_pagesize" layout="prev, pager, next" :total="post_total" style=" text-align: center;">
          </el-pagination>
        </div>
        <div></div>
      </div>
      ...
      <div class="forum-rightcontent">
        <div class="self-show">
          <el-card class="forum-rightcard" style="font-size:18px">
            <div class="block">
              <el-avatar :size="75" :src="my_head" style="margin-left: calc(50% - 37.5px);"></el-avatar>
            </div>
        ...
        <div class="hot topic" style="margin-top:10px">
          <el-card class="rigthcard" v-model="forum_hot">
            ...
            <div v-for="(item, o) in forum_hot" :key="o" class="text item" style="text-align: center;">
              {{ o + 1 + ". " }}{{ item.title }}
        ...
        <div class="list" style="margin-top:10px">
          <el-card class="rigthcard" v-model="forum_top" style="text-align: center;">
...
export default {
  ...
  methods: {
    ...
    get_user_role(){
      var _this = this;
      this.$http.get("http://39.101.122.176:88/user/getuser?username=" + this.forum_username).then(res => {
    ...
    delete_post(id) {
      ...
      var _this=this;
      if (this.role == 2) {
        this.$http.get("http://39.101.122.176:88/forum/post/del?id=" + id).then(res => {
          this.$message({
          message: '删除成功！',
          type: 'success'
        });
    ...
    forum_post() {
      ...
      this.$http
        .get("http://39.101.122.176:88/forum/post/page?page=0&pageSize=5&username=" + document.cookie.replace("username=", ""))
        .then(res => {
          this.posts = res.data.data;
          console.log("1" + this.posts);
          for (var i = 0; i < 5; i++) { this.post_isliked[i].cut = res.data.data[i].islike; }
    ...
    forum_conment_getall(id, comment_currentpage, index) {
      ...
      console.log(index);
      this.$http
        .get("http://39.101.122.176:88/forum/comment/get?id=" + id + "&username=" + this.forum_username)
        .then(res => {
      ...
      if (this.forum_username == "") {
        ...
        this.$http
          .get(
            ...
            _this.posts[index].id + "&username=" + document.cookie.replace("username=", "")
          )
          .then(res => {
            console.log(res);
            if (this.posts[index].islike) { this.posts[index].up -= 1; }
            else { this.posts[index].up += 1; }
          });
      }
      ...
      if (this.forum_username == "") {
        ...
        this.$http
          .get(
            ...
            this.cur_comment.id + "&username=" + document.cookie.replace("username=", "")
          )
          .then(res => {
            console.log(res);
            if (this.comments[index].data[index1].islike) { this.comments[index].data[index1].up -= 1; }
            else { this.comments[index].data[index1].up += 1; }
            this.comments[index].data[index1].islike = !this.comments[index].data[index1].islike;
          });
      }
    },
    get_post_count() {
      ...
      if (this.search == "") {
        this.$http
          .get(
            ...
            "&pageSize=5" + "&username=" + document.cookie.replace("username=", "")
          )
          .then(res => {
        ...
        this.$http
          .get(
            ...
            (_this.post_currentpage - 1) * 5 +
            "&pageSize=5&name=" +
            this.search + "&username=" + document.cookie.replace("username=", "")
          )
          .then(res => {
            _this.posts = res.data.data;
            console.log(_this.posts);
